"""
Bon Commande Repository — access to purchase orders, proformas, invoices.
"""
...
class BonCommandeRepository(BaseRepository):
    """Repository for BonCommande model."""

    model_name = "bc.BonCommande"
    ...
    def filter_by_type(self, type_document: str, limit: int = 50) -> List[Dict[str, Any]]:
        qs = self._get_model().objects.filter(type_document=type_document)
        return self._to_list(qs[:limit])
